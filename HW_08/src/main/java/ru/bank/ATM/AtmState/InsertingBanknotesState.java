...
public class InsertingBanknotesState extends AtmState {
    ...
    public void doAction(AtmContext context) {
        displayMenuMessage(MenuMessages.INSERT_BANKNOTES, context.getATM().getAtmName());
        ...
        if (!notValidBanknotes.isEmpty()) {
            displayMenuMessage(MenuMessages.INVALID_BANKNOTES_INPUT,
                    context.getATM().getAtmName(),
                    String.join(",", notValidBanknotes));
        }
        context.getCurrentClient().getBankCardByNumber(context.getCurrentClientCardNumber()).replenishment(summ);
        displayMenuMessage(MenuMessages.YOU_HAVE_CREDITED_YOUR_CARD,
                context.getATM().getAtmName(),
                String.valueOf(summ));
